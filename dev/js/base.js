...
var GlobalConfigue = {
  masterDomain: "http://www.luwei.space:3000",
};
...
var General = {
  ...
  scrollToPos: function (position) {
    ...
    var button = $(
      '<a href="#" id="to-top" title="' +
        STR_TO_TOP +
        '"> <div class="to-top-wrap"></div></a>'
    ).appendTo("body");
  ...
  urlIconlize: function (url) {
    ...
    var iconMap = {
      /*索引 可在这里添加匹配规则*/ twitter: iconFontTag + "-twitter",
  ...
  addIcons: function () {
    ...
    $(".single-post-inner  a:not(:has(img))").each(function (i) {
      ...
      $(this).prepend(
        '<i class="iconfont ' + General.urlIconlize(_selfDomain) + '"></i>'
      );
  ...
  rewardLoader: function () {
    var loadQR = {
      alipay: "/assets/images/qr-alipay.JPG",
      wechat: "/assets/images/qr-wechat.JPG",
    };
    var loadQRUrl;
    if (!!General.isWechat) {
    ...
    $(".money-like .reward-button").hover(
      function () {
        console.log("悬浮");
  ...
  commentLoader: function () {
    ...
    $(window).scroll(function () {
      if (
        $(".author-image").isOnScreenVisible() &&
        !$(".author-image").hasClass("comment-loaded")
      ) {
        var d = document,
...
var ImageSmartLoader = {
  ...
  webPCheck: function (feature, callback) {
    var TestImages = {
      demo:
        "UklGRkoAAABXRUJQVlA4WAoAAAAQAAAAAAAAAAAAQUxQSAsAAAABBxAREYiI/gcAAABWUDggGAAAADABAJ0BKgEAAQABABwlpAADcAD+/gbQAA==",
    };
    console.log("支持Webp哦");
    var img = new Image();
    img.onload = function () {
    ...
    this.each(function () {
      ...
      $self.one("appear", function () {
        if (!this.loaded) {
          ...
          if (
            updatedUrl.indexOf("upaiyun") > -1 ||
            updatedUrl.indexOf("file.is26.com") > -1
          ) {
            // alert(1)
